...
draw.linear <- function(output, sizefile, ..., directory = NULL, fileformat = "png", anc_colours = NULL, chr_colours = NULL, w = 13, h = 5, opacity = .5, show_chromosome_names = TRUE, threshold=0.80, colour_by_genome = NULL) {
  ...
  # plot the two last genomes next to each other
  # need to change the y position, so first and second genome have y value 0, 
  # and reduce one for all the other

  compiled.size[compiled.size$y > 0, ]$y <- compiled.size[compiled.size$y > 0, ]$y - 2


  # For species sharing the same y level, shift the second species x-coordinates
  # so they appear side-by-side rather than overlapping
  for (y_level in unique(compiled.size$y)) {
    same_y <- unique(compiled.size$species[compiled.size$y == y_level])
    if (length(same_y) >= 2) {
      sp1 <- same_y[1]
      sp2 <- same_y[2]
      x_offset <- max(compiled.size$xend[compiled.size$species == sp1]) + 50000000
      compiled.size$xstart[compiled.size$species == sp2] <-
        compiled.size$xstart[compiled.size$species == sp2] + x_offset
      compiled.size$xend[compiled.size$species == sp2] <-
        compiled.size$xend[compiled.size$species == sp2] + x_offset
    }
  }

  # Centre each species on its own row around the overall x midpoint
  # (skip species sharing a y level — those are already side-by-side)
  overall_mid <- max(compiled.size$xend) / 2
  for (sp in unique(compiled.size$species)) {
    sp_y <- unique(compiled.size$y[compiled.size$species == sp])
    n_species_on_row <- length(unique(compiled.size$species[compiled.size$y == sp_y]))
    if (n_species_on_row > 1) next  # skip side-by-side pairs
    sp_mid <- (min(compiled.size$xstart[compiled.size$species == sp]) +
               max(compiled.size$xend[compiled.size$species == sp])) / 2
    shift <- overall_mid - sp_mid
    compiled.size$xstart[compiled.size$species == sp] <-
      compiled.size$xstart[compiled.size$species == sp] + shift
    compiled.size$xend[compiled.size$species == sp] <-
      compiled.size$xend[compiled.size$species == sp] + shift
  }
  ...
  # Build gene_id -> chr lookup from the colour reference genome
  ...
  for (i in 1:length(listsynt)) {
    ...
    rect_to_shape <- function(df, chr_height = 0.1, chr_width_extra = 0.12) {
    ...
    # Compute species label positions
      # If on the bottom row
    if (tar_y[1] == 0 & min(tar_sizes$xstart) > 10000000 ) {
      #set the label after the last chromosome
      tar_label <- tar_sizes %>% summarise(
        x = max(xend) + 10000000,
        y = unique(y) + 0.05,
        species = unique(species)
      )
    } else {
      # Default: label before first chromosome
      ref_label <- ref_sizes %>% summarise(
        x = min(xstart) - 10000000,
        y = unique(y) + 0.05,
        species = unique(species)
      )
    }
    ...
    p <- p +
      ...
      ggplot2::geom_text(data = ref_label, mapping = ggplot2::aes(x = x, y = y, label = species), size = 4, hjust = 1) +
      ggforce::geom_shape(
        data = tar_poly,
        mapping = ggplot2::aes(x = x, y = y, group = group_id, fill = fill_chr),
        color = "grey20", linewidth = 0.4, radius = unit(2, "pt")
    ...
    if (exists("tar_label")) {
      p <- p + ggplot2::geom_text(data = tar_label, mapping = ggplot2::aes(x = x, y = y, label = species), size = 4, hjust = 0)
    }
    ...
    if (!is.null(anc_colours)) {
      ...
      fill_legend <- paste0("Inferred CARs (", colour_ref_species, ")")
    }
    ...
    p <-  p + 
      geom_polygon(data = ribbon_poly, 
                   aes(x = x, y = y, group = group, fill = chr_colour), colour = NA, alpha = 0.5) + 
